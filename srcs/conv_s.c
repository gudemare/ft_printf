...
/*   Updated: 2017/06/24 04:34:33 by gudemare         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	wchar_t	*apply_wmfw(wchar_t *ws, t_spec spec)
{
	...
	if (!(wbuf = (wchar_t*)malloc((len + 1) * sizeof(wchar_t))))
		exit(EXIT_FAILURE);
...
static	int		conv_ws(va_list ap, t_spec spec)
{
	wchar_t	*wbuf;
	int		ret;

	wbuf = va_arg(ap, wchar_t*);
	if (!(wbuf = ft_wstrdup((wbuf) ? wbuf : L"(null)")))
		exit(EXIT_FAILURE);
	...
	ft_putwnstr(wbuf, ft_wstrlen(wbuf));
...
int				conv_s(va_list ap, t_spec spec)
{
	...
	if (spec.length && ft_strchr(spec.length, 'l'))
		return (conv_ws(ap, spec));
	buf = (char *)va_arg(ap, char*);
	if (!(buf = ft_strdup((buf) ? buf : "(null)")))
		exit(EXIT_FAILURE);
